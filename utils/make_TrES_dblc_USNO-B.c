/*******************************************************************
 * This program find the colors and RA/DEC (from 2MASS), and proper motion (from USNO-B1) of the each LC in a 31-param DEBiL DB.
 * It may also be used to filters through only the light curves that are red  and have proper motions (using USNO-B1).
 * Note that this program shouldn't be run in parallel (unless you set different SCAT_TMP_FILENAMEs).
 *
 * compile: gcc make_TrES_dblc_USNO-B.c -o make_TrES_dblc_USNO-B -Wall -lm -O
 *          gcc make_TrES_dblc_USNO-B.c -o filter_TrES_dbl -Wall -lm -O        [after remarking "#define DO_DBLC"]
 *          gcc make_TrES_dblc_USNO-B.c -o make_TrES_dblc_USNO-B_pm -Wall -lm -O      [with "#define FILTER_OUT_ZERO_PM"]
 *          gcc make_TrES_dblc_USNO-B.c -o filter_TrES_dbl_pm -Wall -lm -O      [remarking "#define DO_DBLC" and with "#define FILTER_OUT_ZERO_PM"]
 *
 * run:     make_TrES_dblc_USNO-B listAll.edit.dbl and0.2mass listAll.edit.dblc
 *
 *******************************************************************/
...
//#define MAX_USNO_MAG_DIFF 1.0 // in magnitudes
//#define MAX_USNO_TrES_MAG_DIFF 1.5 // in magnitudes
...
{ 
  ...
  while (31 == fscanf(finDebil, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9, &x10, &x11, &x12, &x13, &x14, &x15, &x16, 
      ...
      if ((magJ - magH < MIN_JmH) || (magH - magK < MIN_HmK))
	{
      ...
      //------------------ look for USNO-B1.0 proper motion ------------------

      sprintf (strScatFilename, "%s.%s.%ld", argv[3], SCAT_TMP, nameNum) ;

#ifdef LINUX
      sprintf (str,"/data/oir/wcs/bin.redhat/scat -atc ub1 %s %s J2000 | tail -1 > %s", strRA, strDEC, strScatFilename) ;
#else
      sprintf (str,"/data/oir/wcs/bin.solaris/scat -atc ub1 %s %s J2000 | tail -1 > %s", strRA, strDEC, strScatFilename) ;
#endif
      
      system (str) ;
      
      finUSNO = fopen (strScatFilename, "rt") ;
      if (!finUSNO)
	{
	  printf ("Warnnig: Couln't open the USNO file ('%s')\n", strScatFilename) ;
	  continue ;
	}

      if (5 != fscanf (finUSNO, "%*f %*d:%*d:%*f  %*d:%*d:%*f   %*f %f %*f %f %*f  %*d %*d %*d  %f %f %f", &magR1, &magR2, &pmRA, &pmDEC, &ra_dec_err))
	{
	  printf ("Warning: Invalid USNO result parsing ('%s')\n", strScatFilename) ;
	  fclose (finUSNO) ;
      ...
      if (ra_dec_err <= MAX_RA_DEC_ERR)
      	{
	  printf ("Warning: USNO closest star is too far (%f)\n", ra_dec_err) ;
	  continue ;
      	}
      
      if (fabs(magR1 - magR2) > MAX_USNO_MAG_DIFF)
	{
      	  printf ("Warning: USNO R-magnitudes are too different (%f  %f)\n", magR1, magR2) ;
	  continue ;
	}
      
      if (fabs((-2.5 * log10(pow(10.0, -0.4 * x9) + pow(10.0, -0.4 * x11))) - (0.5 * (magR1 + magR2))) > MAX_USNO_TrES_MAG_DIFF)
      	{
      	  printf ("Warning: USNO R-magnitude vary different from TrES (%f   %f %f)\n",
      		  -2.5 * log10(pow(10.0, -0.4 * x9) + pow(10.0, -0.4 * x11)), magR1, magR2) ;
      	  continue ;
      	}
      */
