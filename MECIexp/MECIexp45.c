 ...
 * Configured for DEBiL with color (45-param), over OGLE (I-band)
 *
 * compile:  gcc MECIexp45.c -o MECIexp45  -lm -Wall -O4
 *
 * run:  MECIexp45 ag.dbl
 *
 * if using contour:
 *
...
#define DO_UNCORRECTED_COLOR  // if you suspect the binary is nearby
...
int main(int argc, char **argv)
{
  ...
  int i, j ;
  double mag2mag1, mag2mag1err, VmI, VmIerr, inv_a ;
  double tVmI, tmag2mag1, t_r1, t_r2 ;
  double sigma2VmI, sigma2mag2mag1, sigma2r1, sigma2r2 ;
  //  double JmK, JmKerr, tJmK, BmV, BmVerr, tBmV ; 
  FILE *fin ;
  double x_period, x_r1, x_r1err, x_r2, x_r2err, x_mag1, x_mag1err, x_mag2, x_mag2err, x_chi2 ;
  double x_VmI_corrected, x_VmI_uncorrected, x_Ierr, x_Verr ;
  int x_field, x_obj ;

#ifdef DO_CONTOURS
  FILE *fout_VmI, *fout_mag2mag1, *fout_r1, *fout_r2, *fout_comb, *foutIDL ;
  char nameStr[256] ;
#endif 
  
  if (argc != 2)
    {
      printf (" Usage:  %s  <DEBiL database (45-col)>\n", argv[0]) ;
      return (1) ;
    }
  ...
  while (16 == fscanf (fin, "%d %d %lf %*f %*f %lf %lf %lf %lf %lf %lf %lf %lf %*f %*f %*f %*f %*f %*f %*d %*d %lf %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %lf %*f %lf %lf %lf %*s %*s %*f\n",
		       &x_field, &x_obj, &x_period, &x_r1, &x_r1err, &x_r2, &x_r2err, &x_mag1, &x_mag1err, &x_mag2, &x_mag2err, &x_chi2, &x_VmI_corrected, &x_VmI_uncorrected, &x_Verr, &x_Ierr))
    {
      mag2mag1 = x_mag2 - x_mag1 ;
      mag2mag1err = sqrt(sqr(x_mag1err) + sqr(x_mag2err)) ;

#ifdef DO_UNCORRECTED_COLOR
      VmI = x_VmI_uncorrected ;
#else
      VmI = x_VmI_corrected ;
#endif

      VmIerr =  sqrt(sqr(x_Verr) + sqr(x_Ierr)) ;
      ...
      for (i = 0 ; i < PAIR_ARR_SIZE ; i++)
	{
...
#ifdef DO_CONTOURS  
      sprintf (nameStr, "%s_%d_%d.contour", argv[1], x_field, x_obj) ;

      fout_VmI = contourHeader (nameStr, "V-I", foutIDL) ;
      fout_mag2mag1 = contourHeader (nameStr, "I2-I1", foutIDL) ;
      ...
      if (!fout_VmI || !fout_mag2mag1 || !fout_r1 || !fout_r2 || !fout_comb)
	{
	  printf ("ERROR: couldn't open an output file\n") ;
	  fclose (fin) ;
	  if (fout_VmI) fclose (fout_VmI) ;
	...
	for (j = 0 ; j < NUM_STAR_TYPES ; j++)
	  {
	    ...
	    tVmI = -2.5 * (log10(pow(10.0, -0.4 * star[i].V) + pow(10.0, -0.4 * star[j].V)) -
			   log10(pow(10.0, -0.4 * star[i].I) + pow(10.0, -0.4 * star[j].I))) ; 

	    tmag2mag1 = star[j].I - star[i].I ;
	    ...
	    pair.star1 = i ;
	    pair.star2 = j ;
	    pair.sigma = sqrt(sigma2VmI + sigma2mag2mag1 + sigma2r1 + sigma2r2) ;
      ...
      printf ("ogle%d_%d  P=%.2f chi2=%.2f :   ", x_field, x_obj, x_period, x_chi2) ;
