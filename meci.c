 ...
 *
 * compile:  gcc meci.c debil_lib.c -o meci -lm -Wall -O4
 *           gcc meci.c debil_lib.c -o meci_allParam  -lm -Wall -O4           // unremark:  #define SIMPLEX_SEARCH_ALL_PARAMS
 *           gcc meci.c debil_lib.c -o meci_contour -lm -Wall -O4             // unremark:  #define CONTOUR_OUTPUT_SUFFIX
 *           gcc meci.c debil_lib.c -o meci_allParam_contour  -lm -Wall -O4   // unremark:  #define CONTOUR_OUTPUT_SUFFIX and SIMPLEX_SEARCH_ALL_PARAMS
 *
 * run:    meci q1.dblc LimbDarkTable.txt SunIsochrone.txt q1.meci q1.meci_err
 *
 * plot:
 *
 *   idl
...
#define DEFAULT_CONTOUR_MIN_MASS 0.0
#define DEFAULT_CONTOUR_MAX_MASS 2.5

// if an observed magnitude (of any filter) is provided, will calculate the expected observed mag for the following filter
#define DO_CALC_OBS_MAG 'V'   
...
struct ObsmagInfoType
{
  char filterCh ;
  int filter ;
  double obsmagVal, obsmagErr ;
} ;
...
// Converts a character filter (e.g. 'V') to its corresponding index in arrays
...
// A comparator for a pair-o-double
// Returns: -1 if x < y
//           0 if x = y
//           1 if x > y
int comparDoubles (const void *x, const void *y)
{
...
int contourHeader (char *nameStr, float age, FILE *foutIDL, char *filenameContour,
		   double *contourMatrix, int matrixLen, int *subsetArr, int subsetLen, double maxContourMass)
{
  ...
  fprintf (foutIDL, "contour, z, m, m, /fill, xtitle='Star1 Mass  [M!D!Mn!N]', ytitle='Star2 Mass  [M!D!Mn!N]', title='%s   Age=%.3fGyr', xrange=[%.4f,%.4f], yrange=[%.4f,%.4f], ",
	   nameStr, age, DEFAULT_CONTOUR_MIN_MASS, maxContourMass, DEFAULT_CONTOUR_MIN_MASS, maxContourMass) ;
  ...
  fprintf (foutIDL, "contour, z, m, m, /follow, /overplot, xrange=[%.4f,%.4f], yrange=[%.4f,%.4f], ",
	   DEFAULT_CONTOUR_MIN_MASS, maxContourMass, DEFAULT_CONTOUR_MIN_MASS, maxContourMass) ;
  fprintf (foutIDL, "levels=[0") ;
  for (i = 0 ; i < NUM_CONTOUR_LEVELS ; i++)
    fprintf (foutIDL, ",%s", labels[i]) ;
		  ...
		  int dataLimbDarkSliceSize, double *w)
{
  ...
  if (dataLimbDarkSliceSize < 2)
    return (-1) ;

  if (dataLimbDarkSlice[0] > x)
    {
      imin = 0 ;
      imax = 1 ;
    }
  else if (dataLimbDarkSlice[dataLimbDarkSliceSize - 1] < x)
    {
      imin = dataLimbDarkSliceSize - 2 ;
      imax = dataLimbDarkSliceSize - 1 ;
    }
  else
    {
      imin = 0 ;
...
// returns:  0 = success ; otherwise failure 
// Assumes that the given array is sorted (monotonically rising)
int interpolateLimbDark (int indexLCfilter, double xLogg, double xTeff, struct LimbDarkType *plimbDark,
			 double *pcoef_a, double *pcoef_b)
{
  ...
  if (iLogg < 0) iLogg = 0 ;
  if (iLogg > (NUM_LOGG-2)) iLogg = NUM_LOGG - 2 ;

  wLoggComp = xLogg - iLogg ;
  wLogg = 1.0 - wLoggComp ;

  if ((iLogg < 0) || ((iLogg+1) >= NUM_LOGG))   // this should never happen
    {
      printf ("ERROR: interpolateLimbDark1  %d  %f\n", iLogg, xLogg / 2.0) ;
      return (1) ;
    }
  ...
  if (iTeff1 < 0)
    {
      // printf ("interpolateLimbDark2  %d  %f\n", iTeff1, xTeff) ;
      return (100 - iTeff1) ;
    }
  ...
  if (iTeff2 < 0)
    {
      // printf ("interpolateLimbDark3  %d  %f\n", iTeff2, xTeff) ;
      return (200 - iTeff2) ;
    }
  ...
  *pcoef_a =
    (wLogg * ((wTeff1 * plimbDark->coef_a[iLogg][indexLCfilter][iTeff1]) +
	      (wTeff1comp * plimbDark->coef_a[iLogg][indexLCfilter][iTeff1+1]))) +
    (wLoggComp * ((wTeff2 * plimbDark->coef_a[iLogg+1][indexLCfilter][iTeff2]) +
		  (wTeff2comp * plimbDark->coef_a[iLogg+1][indexLCfilter][iTeff2+1]))) ;
  
  *pcoef_b =
    (wLogg * ((wTeff1 * plimbDark->coef_b[iLogg][indexLCfilter][iTeff1]) +
	      (wTeff1comp * plimbDark->coef_b[iLogg][indexLCfilter][iTeff1+1]))) +
    (wLoggComp * ((wTeff2 * plimbDark->coef_b[iLogg+1][indexLCfilter][iTeff2]) + 
		  (wTeff2comp * plimbDark->coef_b[iLogg+1][indexLCfilter][iTeff2+1]))) ;
...
int isUnobservableDip(double R1, double Mag1, double R2, double Mag2)
{
  ...
  maxDip1 = 2.5 * log10 (1.0 + (sqr(R2/R1) / (1.0 - sqr(R2/R1) + pow(10.0, 0.4 * (Mag1 - Mag2))))) ;
  maxDip2 = 2.5 * log10 (1.0 + pow(10.0, 0.4 * (Mag1 - Mag2))) ;
...
// Prints the best-fit sin(i) result.
void printEclipseInfo (int res, double phase1, double dmag1, double phase2, double dmag2,
		       double period, double midTime, char *errFilename, char *dataFilename)
{
  char strErr1[MAX_STRLEN], strErr2[MAX_STRLEN] ;

  if (res)
    {
#ifdef SET_ZERO_ECCENTRICITY
      printError (errFilename, 0, dataFilename, "findEclipses_noEcc()", "Couldn't locate the eclipses") ;
#else
      printError (errFilename, 0, dataFilename, "findEclipses()", "Couldn't locate the eclipses") ;
#endif
      return ;
    }
  
  if (period == 0.0)  // shouldn't happen
    {
#ifdef SET_ZERO_ECCENTRICITY
      printError (errFilename, 0, dataFilename, "findEclipses_noEcc()", "P=0") ;
#else
      printError (errFilename, 0, dataFilename, "findEclipses()", "P=0") ;
#endif
      return ;
    }

  sprintf (strErr1, "M1 eclipsed: dmag=%.3f  phase=%.3f  epoch=%.3f", 
	   dmag1, phase1, period * (floor(midTime / period) + phase1)) ;
  sprintf (strErr2, "M2 eclipsed: dmag=%.3f  phase=%.3f  epoch=%.3f",
	   dmag2, phase2, period * (floor(midTime / period) + phase2)) ;

#ifdef SET_ZERO_ECCENTRICITY
  printError (errFilename, -1, dataFilename, "findBestSini_noEcc()", strErr1) ;
  printError (errFilename, -1, dataFilename, "findBestSini_noEcc()", strErr2) ;
#else
  printError (errFilename, -1, dataFilename, "findBestSini()", strErr1) ;
  printError (errFilename, -1, dataFilename, "findBestSini()", strErr2) ;
#endif
}
...
double findBestSini (float *time, float *amp, float *err, int size, double e,
		     ...
		     double quadA1, double quadB1, double quadA2, double quadB2, double period, double midTime,
		     FILE *foutFit, FILE *foutDat, char *errFilename, char *dataFilename)
{
  ...
  double phase1, dmag1, phase2, dmag2 ;
  ...
  if (sini_1_chi2 < getChi2 (time, amp, err, size, e, r1, r2, mag1, mag2, sini_max, time0, omega, quadA1, quadB1, quadA2, quadB2))
      ...
      if (errFilename)
	{
	  ...
	  res = findEclipses (e, r1, r2, mag1, mag2, 1.0, time0, omega, quadA1, quadB1, quadA2, quadB2,
			      &phase1, &dmag1, &phase2, &dmag2) ;
	  printEclipseInfo (res, phase1, dmag1, phase2, dmag2, period, midTime, errFilename, dataFilename) ;
	}
  ...
  if (errFilename)
    {
      ...
      res = findEclipses (e, r1, r2, mag1, mag2, sini_mid, time0, omega, quadA1, quadB1, quadA2, quadB2,
			  &phase1, &dmag1, &phase2, &dmag2) ;
      printEclipseInfo (res, phase1, dmag1, phase2, dmag2, period, midTime, errFilename, dataFilename) ;
    }
			   ...
			   FILE *foutFit, FILE *foutDat, char *errFilename, char *dataFilename)
{
  ...
  if (sini_1_chi2 < getChi2noEcc (time, amp, err, size, r1, r2, mag1, mag2, sini_max, time0, quadA1, quadB1, quadA2, quadB2))
      ...
      if (errFilename)
	{
	  ...
	  res = findEclipses_noEcc (r1, r2, mag1, mag2, 1.0, time0, quadA1, quadB1, quadA2, quadB2,
				    &phase1, &dmag1, &phase2, &dmag2) ;
	  printEclipseInfo (res, phase1, dmag1, phase2, dmag2, period, midTime, errFilename, dataFilename) ;
	}
  ...
  if (errFilename)
    {
      ...
      res = findEclipses_noEcc (r1, r2, mag1, mag2, sini_mid, time0, quadA1, quadB1, quadA2, quadB2,
				&phase1, &dmag1, &phase2, &dmag2) ;
      printEclipseInfo (res, phase1, dmag1, phase2, dmag2, period, midTime, errFilename, dataFilename) ;
    }
			 ...
			 FILE *foutFit, FILE *foutDat, char *errFilename, char *dataFilename)
{
  ...
  if (errFilename)
    {
      sprintf (strErr, "Sin(i)=%f  e=%f  time0=%f  omega=%f",
	       p[ilo][D_SIN_I], p[ilo][D_ECC], p[ilo][D_TPO] + p[ilo][D_TMO], 2.0 * M_PI * (p[ilo][D_TPO] - p[ilo][D_TMO])) ;
...
double getScore (int g, int i, int j, int indexLCfilter, double period, double splineScore,
		 float *time, float *amp, float *err, int size, double combObsMag,
		 double e, double t0, double omega, float LCweighting, double midTime, 
		 ...
		 FILE *foutFit, FILE *foutDat, char *errFilename, char *dataFilename)
{
  ...
  calcMags (combObsMag, starGroup[g].Mag[indexLCfilter][j] - starGroup[g].Mag[indexLCfilter][i], &mag1, &mag2) ;
...
#ifdef MIN_OBSERVABLE_DIP_DEPTH
  if (isUnobservableDip(starGroup[g].radius[i], starGroup[g].Mag[indexLCfilter][i],
			starGroup[g].radius[j], starGroup[g].Mag[indexLCfilter][j]))
  ...
  if (interpolateLimbDark (indexLCfilter, xLogg, starGroup[g].Teff[i], plimbDark, &quadA1, &quadB1))
    return (RESULT_OUT_OF_BOUND_LIMB_DARK) ;
  
  xLogg = 4.437542 + log10(starGroup[g].mass[j] / sqr(starGroup[g].radius[j])) ; // 4.437542 = log(g_sol)
  if (interpolateLimbDark (indexLCfilter, xLogg, starGroup[g].Teff[j], plimbDark, &quadA2, &quadB2))
    return (RESULT_OUT_OF_BOUND_LIMB_DARK) ;
  ...
  if (LCweighting != 0.0)
    {
      ...
      score = findBestSini_noEcc (time, amp, err, size, r1, r2, mag1, mag2, t0 - (omega / 360.0),
				  quadA1, quadB1, quadA2, quadB2, period, midTime, foutFit, foutDat, errFilename, dataFilename) ;
#else
#ifdef SIMPLEX_SEARCH_ALL_PARAMS
      score = findBestAllParam (time, amp, err, size, e, r1, r2, mag1, mag2, t0, omega, quadA1,
				quadB1, quadA2, quadB2, period, midTime, foutFit, foutDat, errFilename, dataFilename) ;
#else
...
double greedyFindMin (int g, int *pi_best, int *pj_best, int indexLCfilter, double period, double splineScore,
		      float *time, float *amp, float *err, int size, double combObsMag,
		      double e, double t0, double omega, float LCweighting, double midTime,
		      struct LimbDarkType *plimbDark, struct ColorInfoType *colorInfoArr, int numColors,
		      struct StarGroupType starGroup[MAX_NUM_STAR_GROUPS], 
		      double *contourMatrix, int numLen)
{
  ...
  do
	...
	{
	  score = getScore (g, i-1, j, indexLCfilter, period, splineScore, time, amp, err, size,
			    combObsMag, e, t0, omega, LCweighting, midTime,
	...
	{
	  score = getScore (g, i, j-1, indexLCfilter, period, splineScore, time, amp, err, size,
			    combObsMag, e, t0, omega, LCweighting, midTime,
			    plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
	...
	{
	  score = getScore (g, i-1, j+1, indexLCfilter, period, splineScore, time, amp, err, size,
			    combObsMag, e, t0, omega, LCweighting, midTime, 
			    plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
...
// Fits neighboring mass/age samples to a paraboloid function:   score = f(mass1, mass2, age)
// such that  f(x,y,z) = A1*x^2 + A2*x + A3*y^2 + A4*y + A5*z^2 + A6*z + A7
// then it uses the parameters to estimate the location (*pmass1, *pmass2, *page) 
// and depth of the minimum (*pminimumOfMassAge), and to estimate the uncertainties (*pmass1Err, *pmass2Err, *pageErr)
// IMPORTANT: best_g, best_i, and best_j must NOT be at the edge of the available range
void calcParabMassAge (int best_g, int best_i, int best_j, int indexLCfilter, double period, double splineScore,
		       float *time, float *amp, float *err, int size, double combObsMag,
		       double e, double t0, double omega, float LCweighting, double midTime,
		       struct LimbDarkType *plimbDark, struct ColorInfoType *colorInfoArr, int numColors,
		       struct StarGroupType starGroup[MAX_NUM_STAR_GROUPS],
		       char *errFilename, char *deb_filename,
		       double *pmass1, double *pmass1Err, double *pmass2, double *pmass2Err,
		       double *page, double *pageErr, double *pminimumOfMassAge)
{ 
  ...
  coord[1][0] = getScore (best_g, best_i+1, best_j, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			  e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;

  coord[2][1] = starGroup[best_g].mass[best_i-1] ;
  coord[2][2] = starGroup[best_g].mass[best_j] ;
  coord[2][3] = starGroup[best_g].age ;
  coord[2][0] = getScore (best_g, best_i-1, best_j, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			  e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;

  coord[3][1] = starGroup[best_g].mass[best_i] ;
  coord[3][2] = starGroup[best_g].mass[best_j+1] ;
  coord[3][3] = starGroup[best_g].age ;
  coord[3][0] = getScore (best_g, best_i, best_j+1, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			  e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;

  coord[4][1] = starGroup[best_g].mass[best_i] ;
  coord[4][2] = starGroup[best_g].mass[best_j-1] ;
  coord[4][3] = starGroup[best_g].age ;
  coord[4][0] = getScore (best_g, best_i, best_j-1, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			  e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
  ...
  for (i = 0 ; i < MASS_PARABOLA_FIT_NUM ; i++) 
    if (coord[i][0] >= MAX_VALID_SCORE)
      {
	sprintf (strErr, "Bad neighboring position (%d %.1f)", i, coord[i][0]) ;
	printError (errFilename, 0, deb_filename, "calcParabMassAge()", strErr) ;
  ...
  // ---------- calc paraboloid fit (mass1-mass2-age) ----------
  ...
  if ((vec[0] <= 0.0) || (vec[2] <= 0.0) || (vec[4] <= 0.0))     // shouldn't happen
    {
      printError (errFilename, 0, deb_filename, "calcParabMassAge()", "One of the axes is convex (has a maximum instead of a minimum).") ;
      goto mass_and_mag_with_coord_error_est_failed ;
    }

  *pminimumOfMassAge = vec[6] - (0.25 * ((vec[1]*vec[1]/vec[0]) + (vec[3]*vec[3]/vec[2]) + (vec[5]*vec[5]/vec[4]))) ;

  if (*pminimumOfMassAge < 0.0)
    printError (errFilename, 0, deb_filename, "calcParabMassAge()", "Fitted parabola has a negative minimum score.") ;

  *pmass1 = -0.5 * vec[1] / vec[0] ;
  *pmass1Err = sqrt(1.0 / vec[0]) ;
  *pmass2 = -0.5 * vec[3] / vec[2] ;
  *pmass2Err = sqrt(1.0 / vec[2]) ;
  *page = -0.5 * vec[5] / vec[4] ;
  *pageErr = sqrt(1.0 / vec[4]) ;

  return ;

  //---------------------------------------
  
 mass_and_mag_with_coord_error_est_failed:
 
  *pmass1 = starGroup[best_g].mass[best_i] ;
  *pmass2 = starGroup[best_g].mass[best_j] ; 
  *page = starGroup[best_g].age ;
  *pminimumOfMassAge = starGroup[best_g].bestSliceScore ;

  if ((coord[1][0] >= MAX_VALID_SCORE) && (coord[2][0] < MAX_VALID_SCORE))
    *pmass1Err = UNCERTAINTY_UPPER_LIMIT ;
  else
    if ((coord[2][0] >= MAX_VALID_SCORE) && (coord[1][0] < MAX_VALID_SCORE))
      *pmass1Err = UNCERTAINTY_LOWER_LIMIT ;
    else
      *pmass1Err = UNCERTAINTY_UNKNOWN ;
  
  if ((coord[3][0] >= MAX_VALID_SCORE) && (coord[4][0] < MAX_VALID_SCORE))
    *pmass2Err = UNCERTAINTY_UPPER_LIMIT ;
  else
    if ((coord[4][0] >= MAX_VALID_SCORE) && (coord[3][0] < MAX_VALID_SCORE))
      *pmass2Err = UNCERTAINTY_LOWER_LIMIT ;
    else
      *pmass2Err = UNCERTAINTY_UNKNOWN ;
  
  if ((coord[5][0] >= MAX_VALID_SCORE) && (coord[6][0] < MAX_VALID_SCORE))
    *pageErr = UNCERTAINTY_UPPER_LIMIT ;
  else 
    if ((coord[6][0] >= MAX_VALID_SCORE) && (coord[5][0] < MAX_VALID_SCORE))
      *pageErr = UNCERTAINTY_LOWER_LIMIT ;
    else
      *pageErr = UNCERTAINTY_UNKNOWN ;
}



// Given a filter (indexCombMagFilter), fits neighboring combined absolute magnitude samples to a parabola function:   score = f(mag)
// such that  f(x) = A1*x^2 + A2*x + A3
// then it uses the parameters to estimate the location (*pcombAbsMag) 
// and depth of the minimum (*pminimumOfMag), and to estimate the uncertainties (*pcombAbsMagErr)
// IMPORTANT: best_g, best_i, and best_j must NOT be at the edge of the available range
void calcParabolaMag (int indexCombMagFilter,
		      int best_g, int best_i, int best_j, int indexLCfilter, double period, double splineScore,
		      float *time, float *amp, float *err, int size, double combObsMag,
		      double e, double t0, double omega, float LCweighting, double midTime, 
		      struct LimbDarkType *plimbDark, struct ColorInfoType *colorInfoArr, int numColors,
		      struct StarGroupType starGroup[MAX_NUM_STAR_GROUPS],
		      char *errFilename, char *deb_filename,
		      double *pcombAbsMag, double *pcombAbsMagErr, double *pminimumOfMag)
{
  double coord[MAGNITUDE_PARABOLA_FIT_NUM][4] ;   // 0=score 1=X  2=Y  3=Z  
  double fitMatrix[MAX_MATRIX_SIZE][MAX_MATRIX_SIZE] ;  // row num, col num
  double vec[MAGNITUDE_PARABOLA_FIT_NUM] ;
  int i, j ;
  char strErr[MAX_STRLEN] ;

  coord[0][1] = combineMag(starGroup[best_g].Mag[indexCombMagFilter][best_i],
			   starGroup[best_g].Mag[indexCombMagFilter][best_j]) ;
  coord[0][0] = starGroup[best_g].bestSliceScore ;
  
  coord[1][1] = combineMag(starGroup[best_g].Mag[indexCombMagFilter][best_i+1],
			   starGroup[best_g].Mag[indexCombMagFilter][best_j+1]) ;
  coord[1][0] = getScore(best_g, best_i+1, best_j+1, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			 e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
  
  coord[2][1] = combineMag(starGroup[best_g].Mag[indexCombMagFilter][best_i-1],
			   starGroup[best_g].Mag[indexCombMagFilter][best_j-1]) ;
  coord[2][0] = getScore(best_g, best_i-1, best_j-1, indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
			 e, t0, omega, LCweighting, midTime, plimbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
  ...
  for (i = 0 ; i < MAGNITUDE_PARABOLA_FIT_NUM ; i++) 
    if (coord[i][0] >= MAX_VALID_SCORE)
      {
	sprintf (strErr, "Bad neighboring position (%d %d %.1f)", indexCombMagFilter, i, coord[i][0]) ;
	printError (errFilename, 0, deb_filename, "calcParabolaMag()", strErr) ;
  ...
  for (i = 1 ; i < MAGNITUDE_PARABOLA_FIT_NUM ; i++)
    if (coord[i][0] < coord[0][0])
      {
	sprintf (strErr, "The given best point is not the minimum (%d %f %f %f %d)", indexCombMagFilter,
  ...
  if (vec[0] <= 0.0)
    {
      printError (errFilename, 0, deb_filename, "calcParabolaMag()", "The axis is convex (has a maximum instead of a minimum).") ;

      goto mag_error_est_failed ;
    }

  *pminimumOfMag = vec[2] - (0.25*vec[1]*vec[1]/vec[0]) ;

  if (*pminimumOfMag < 0.0)
    printError (errFilename, 0, deb_filename, "calcParabolaMag()", "Fitted parabola has a negative minimum score.") ;

  *pcombAbsMag = -0.5 * vec[1] / vec[0] ;
  *pcombAbsMagErr = sqrt(1.0 / vec[0]) ;

  return ;

  //-------------------------------------

 mag_error_est_failed:

  *pcombAbsMag = combineMag(starGroup[best_g].Mag[indexCombMagFilter][best_i],
			    starGroup[best_g].Mag[indexCombMagFilter][best_j]) ;
  *pcombAbsMagErr = UNCERTAINTY_UNKNOWN ;
  *pminimumOfMag = starGroup[best_g].bestSliceScore ;
}


// Given the location of the best pairing (best_g, best_i, best_j), will interpolate a paraboloid
// and find a more accurate best-pairing, between the isochron data. The width of the paraboloid
// is used to calculate the formal uncertinaties.
// Note that if the given best location is at the edge of the range, no interpolation is done, 
// and many or all of the uncertainties will be upper/lower limits or unknown.
void parabolaFit (int best_g, int best_i, int best_j, int indexLCfilter, double period, double splineScore,
		  float *time, float *amp, float *err, int size, double combObsMag,
		  double e, double t0, double omega, float LCweighting, double midTime,
		  char *strRA, char *strDEC, float pmRA, float pmDEC, float ra_dec_err,
		  struct LimbDarkType *plimbDark, 
		  struct ObsmagInfoType *obsmagInfoArr, int numObsmag,
		  struct ColorInfoType *colorInfoArr, int numColors,
		  struct StarGroupType starGroup[MAX_NUM_STAR_GROUPS],
		  int numGroups, char *errFilename, char *deb_filename, FILE *foutResults)
{
  int n ;
  double mass1, mass1Err, mass2, mass2Err, age, ageErr, minimumOfMassAge ;
  double combAbsMag, combAbsMagErr, minimumOfMag ;
  double *arrDistModulus = 0 ;
#ifdef DO_CALC_OBS_MAG
  int filterToCalc = colorConvert (DO_CALC_OBS_MAG) ;
  double medianDistModulus ;
  
  if (numObsmag > 0)
    {      
      if (filterToCalc >= 0)
	{
	  arrDistModulus = (double*)malloc (numObsmag * sizeof(double)) ;
	  if (!arrDistModulus)
	    printError (errFilename, 0, deb_filename, "parabolaFit()", "Memory allocation failure") ;
	}
      else
	printError (errFilename, 0, deb_filename, "parabolaFit()", "Invalid filter to calculate") ;
    }
#endif
  ...
  if ((best_i == 0) || (best_i == (starGroup[best_g].arrLen - 1)) ||
    ...
    {  
      ...
      printError (errFilename, 0, deb_filename, "parabolaFit()", "The best-pairing is at the edge of the range.") ;
	...
	if (best_g == (numGroups - 1))
      ...
      combAbsMag = combineMag(starGroup[best_g].Mag[indexLCfilter][best_i],
			      starGroup[best_g].Mag[indexLCfilter][best_j]) ;
      ...
      // -------------- write distance modulii -----------------------------

      if (numObsmag > 0)
	{      
	  for (n = 0 ; n < numObsmag ; n++)
	    {
	      combAbsMag = combineMag(starGroup[best_g].Mag[obsmagInfoArr[n].filter][best_i],
				      starGroup[best_g].Mag[obsmagInfoArr[n].filter][best_j]) ;
	      if (n)
		fprintf (foutResults, "   ") ;
	      
	      fprintf (foutResults, "%c-filter: Mag=%.3f mag=%.3f", obsmagInfoArr[n].filterCh, combAbsMag, obsmagInfoArr[n].obsmagVal) ;
	      
	      if (arrDistModulus)
		arrDistModulus[n] = obsmagInfoArr[n].obsmagVal - combAbsMag ;
	    }
	  
#ifdef DO_CALC_OBS_MAG
	  if (arrDistModulus)
	    {
	      combAbsMag = combineMag(starGroup[best_g].Mag[filterToCalc][best_i],
				      starGroup[best_g].Mag[filterToCalc][best_j]) ;
	      
	      qsort (arrDistModulus, numObsmag, sizeof(double), comparDoubles) ;
	      
	      if (numObsmag % 2)   // is odd
		medianDistModulus = combAbsMag + arrDistModulus[(numObsmag-1)/2] ;
	      else                 // is even
		medianDistModulus = combAbsMag + (0.5 * (arrDistModulus[(numObsmag-2)/2] + arrDistModulus[numObsmag/2])) ;
	      
	      fprintf (foutResults, "   calculated %c-filter: Mag=%.3f mag=%.3f", DO_CALC_OBS_MAG, combAbsMag, medianDistModulus) ;
  ...
  //================ NOT at the edge of the range =====================================
      
  calcParabMassAge (best_g, best_i, best_j, indexLCfilter, period, splineScore,
		    time, amp, err, size, combObsMag, e, t0, omega, LCweighting, midTime,
		    plimbDark, colorInfoArr, numColors, starGroup, errFilename, deb_filename,
		    &mass1, &mass1Err, &mass2, &mass2Err, &age, &ageErr, &minimumOfMassAge) ;
  ...
  if ((mass1 < starGroup[best_g].mass[best_i-1]) ||
    ...
    printError (errFilename, 0, deb_filename, "parabolaFit()", "Parabola minimum is out of bound.") ;
    
  calcParabolaMag (indexLCfilter, best_g, best_i, best_j, indexLCfilter, period, splineScore,
		   time, amp, err, size, combObsMag, e, t0, omega, LCweighting, midTime,
		   plimbDark, colorInfoArr, numColors, starGroup, errFilename, deb_filename,
  ...
  if (numObsmag > 0)
    {      
      for (n = 0 ; n < numObsmag ; n++)
	{
	  calcParabolaMag (obsmagInfoArr[n].filter, best_g, best_i, best_j, indexLCfilter, period, splineScore,
			   time, amp, err, size, combObsMag, e, t0, omega, LCweighting, midTime,
			   plimbDark, colorInfoArr, numColors, starGroup, errFilename, deb_filename,
			   &combAbsMag, &combAbsMagErr, &minimumOfMag) ;
	  if (n)
      ...
      if (arrDistModulus)
	{
	  calcParabolaMag (filterToCalc, best_g, best_i, best_j, indexLCfilter, period, splineScore,
			   time, amp, err, size, combObsMag, e, t0, omega, LCweighting, midTime,
			   plimbDark, colorInfoArr, numColors, starGroup, errFilename, deb_filename,
			   &combAbsMag, &combAbsMagErr, &minimumOfMag) ;
...
{
  int g, i, j, n, numGroups, size, numObsmagColors, numObsmag, numColors, indexLCfilter ;
  int isInvalidObsmagColorFormat, numElements, numElements_orig ;
  char deb_filename[MAX_STRLEN], strErr[MAX_STRLEN], deb_filenamePeriod[MAX_STRLEN] ;
  double score, splineScore, combObsMag, maxContourMass, midTime ;
  ...
  double obsmagColorVal, obsmagColorErr ;
  char colorLC, strObsmagColor[MAX_STRLEN], strRA[MAX_STRLEN], strDEC[MAX_STRLEN] ;
  ...
  FILE *finDebil, *foutResults, *foutFit, *foutDat ; 
  struct StarGroupType starGroup[MAX_NUM_STAR_GROUPS] ;
  struct ObsmagInfoType *obsmagInfoArr ;
  ...
  if ((argc < 6) || (argc > 9))
    {
      printf ("\n Usage:  %s  <DEBiL DB (31-param) /w color>  <limb-darkening DB>  <isochrone DB>  <output result file>  <output error file>  [num elements* (default=%d)]  [LC weighting** (default=%.2f)]  [contour plot max mass (default=%.2f)]\n",
	      argv[0], DEFAULT_NUM_RESOLUTION_ELEMENTS, DEFAULT_LC_WEIGHTING, DEFAULT_CONTOUR_MAX_MASS) ;
      printf ("Notes:          {version %.1f}\n", VERSION) ;
      printf (" *If num elements is set to zero, MECI will use the maximum available, otherwise it must be at least 2.\n") ;
  ...
  if ((argc < 6) || (argc > 8))
    {
      printf ("\n Usage:  %s  <DEBiL DB (31-param) /w color>  <limb-darkening DB>  <isochrone DB>  <output result file>  <output error file>  [num elements* (default=%d)]  [LC weighting** (default=%.2f)]\n",
  ...
  if (argc >= 7)
    {
      ...
      if (numElements_orig == 0)
	{
	  printf ("Note: Will use all the mass-elements available for each slice.\n") ;
	}
      else if (numElements_orig < 2)
	{
  ...
  if (argc >= 9)
    {
      maxContourMass = atof (argv[8]) ;
      
      if (maxContourMass <= DEFAULT_CONTOUR_MIN_MASS)
	printf ("ERROR: The maximum mass in the contour plot must be greater than %f.\n", DEFAULT_CONTOUR_MIN_MASS) ;
      
      return (4) ;
    }
  else maxContourMass = DEFAULT_CONTOUR_MAX_MASS ;
  ...
  while (7 == fscanf (finDebil, "%s %lf %lf %*f %*f %*f %*f %*f %lf %*f %lf %*f %*f %*f %lf %*f %lf %*f %*d %*d %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f %*f\n", deb_filename, &period, &ecc, &deb_mag1, &deb_mag2, &t0, &omega))
	...
	if (deb_filename[i] == '/')
      ...
      isInvalidObsmagColorFormat = 0 ;   // we use a this since you don't want to break and lose the file sync.
      numObsmag = 0 ;
      numColors = 0 ;
      obsmagInfoArr = 0 ;
      colorInfoArr = 0 ;
      ...
      // read color info:
      if (7 != fscanf (finDebil, "%c %d  %s  %s  %f %f %f\n", &colorLC, &numObsmagColors, strRA, strDEC, &pmRA, &pmDEC, &ra_dec_err))
	{
	  printError (argv[5], 1, deb_filenamePeriod, "main()", "Invalid LC color header.") ;
	  break ;
	}
      
      indexLCfilter = colorConvert (colorLC) ;      
      if (indexLCfilter < 0)
	{
	  printError (argv[5], 1, deb_filenamePeriod, "main()", "Invalid LC color.") ;
	  for (i = 0 ; i < numObsmagColors ; i++)
	    nextLine (finDebil) ;
	  continue ;
	}

      if (numObsmagColors < 0)
	{
	  printError (argv[5], 1, deb_filenamePeriod, "main()", "Invalid number of colors.") ;
	  continue ;
	}
      else if (numObsmagColors == 0)
	{
	  if (LCweighting == 0.0) continue ;  // to prevent a division by zero
	}
      else // if (numObsmagColors > 0)
	{
	  obsmagInfoArr = (struct ObsmagInfoType*)malloc (numObsmagColors * sizeof(struct ObsmagInfoType)) ;
	  colorInfoArr = (struct ColorInfoType*)malloc (numObsmagColors * sizeof(struct ColorInfoType)) ;

	  if (!obsmagInfoArr || !colorInfoArr)
	    {
	      if (obsmagInfoArr) free (obsmagInfoArr) ;
	      if (colorInfoArr) free (colorInfoArr) ;

	      printError (argv[5], 1, deb_filenamePeriod, "main()", "Couldn't allocate obsmagInfoArr or colorInfoArr.") ;
	      for (i = 0 ; i < numObsmagColors ; i++)
		nextLine (finDebil) ;
	      continue ;
	    }
	  
	  for (i = 0 ; (i < numObsmagColors) && !feof(finDebil) ; i++)
	    {
	      if (3 != fscanf (finDebil, "%s %lf %lf\n", strObsmagColor, &obsmagColorVal, &obsmagColorErr))
		isInvalidObsmagColorFormat = 1 ;

	      n = strlen(strObsmagColor) ;
	      
	      if (n == 3)  // Color
		{
		  if (obsmagColorErr <= 0.0)
		    isInvalidObsmagColorFormat = 1 ;

		  colorInfoArr[numColors].filterPos = colorConvert (strObsmagColor[0]) ;
		  colorInfoArr[numColors].filterNeg = colorConvert (strObsmagColor[2]) ;
		  colorInfoArr[numColors].colorVal = obsmagColorVal ;
		  colorInfoArr[numColors].colorErr = obsmagColorErr ;
		   
		  if ((colorInfoArr[numColors].filterPos >= 0) && (colorInfoArr[numColors].filterNeg >= 0))
		    {
		      printf ("color #%d:  (%s) = %f +/- %f\n",
			      numColors+1, strObsmagColor, colorInfoArr[numColors].colorVal, colorInfoArr[numColors].colorErr) ;

		      numColors++ ;
		    }
		  else isInvalidObsmagColorFormat = 1 ;
		}
	      else if (n == 1)  // Observed magnitude
		{  
		  obsmagInfoArr[numObsmag].filterCh = strObsmagColor[0] ;
		  obsmagInfoArr[numObsmag].filter = colorConvert (strObsmagColor[0]) ;
		  obsmagInfoArr[numObsmag].obsmagVal = obsmagColorVal ;
		  obsmagInfoArr[numObsmag].obsmagErr = obsmagColorErr ;
		  
		  if (obsmagInfoArr[numObsmag].filter >= 0)
		    {
		      printf ("obs. mag #%d: (%s) = %f +/- %f\n",
			      numObsmag+1, strObsmagColor, obsmagInfoArr[numObsmag].obsmagVal, obsmagInfoArr[numObsmag].obsmagErr) ;
		      
		      numObsmag++ ;
		    }
		  else isInvalidObsmagColorFormat = 1 ;
		} 
	      else isInvalidObsmagColorFormat = 1 ;  // (n!=1) && (n!=3)
	    }
	  
	  if (isInvalidObsmagColorFormat)
	    {
	      printError (argv[5], 1, deb_filenamePeriod, "main()", "Invalid obs. mag or color format.") ;
	      if (colorInfoArr) free (colorInfoArr) ;
	      if (obsmagInfoArr) free (obsmagInfoArr) ;
	      continue ;
	    }
      ...
      size = loadLC_midTime (deb_filename, period, argv[5], &time, &amp, &err, &midTime) ; 
      ...
      for (g = 0 ; g < numGroups ; g++)
	{
	  ...
	  else if (numElements_orig > starGroup[g].arrLen)
	    {
	      sprintf (strErr, "age=%f: Decreased the number of elements to the size of the mass array (from %d to %d)", 
		       starGroup[g].age, numElements_orig, starGroup[g].arrLen) ;
	      printError (argv[5], 0, deb_filenamePeriod, "main()", strErr) ;
	  ...
	  for (i = 0 ; i < numElements ; i++)
	    for (j = 0 ; j < numElements ; j++)
	      {	
		score = getScore (g, subsetArr[i], subsetArr[j], indexLCfilter, period, splineScore, time, amp, err, size, combObsMag,
				  ecc, t0, omega, LCweighting, midTime, &limbDark, colorInfoArr, numColors, starGroup, 0, 0, 0, 0) ;
	  ...
	  for (i = 0 ; i < numElements ; i++)
		...
		{		  
		  ...
		  bestBasinScore =
		    greedyFindMin (g, &search_i, &search_j, indexLCfilter, period, splineScore, 
				   time, amp, err, size, combObsMag, ecc, t0, omega, LCweighting, midTime,
	  ...
	  if (bestSliceScore < MAX_VALID_SCORE)
	      ...
	      if (!foutContourData)
		{
	      ...
	      if (contourHeader(deb_filenamePeriod, starGroup[g].age, foutIDL, filenameContour,
				contourMatrix, starGroup[g].arrLen, subsetArr, numElements, maxContourMass))
		{
	...
	{
	  parabolaFit (best_g, best_i, best_j, indexLCfilter, period, splineScore, time, amp, err, size, 
		       combObsMag, ecc, t0, omega, LCweighting, midTime, strRA, strDEC, pmRA, pmDEC, ra_dec_err,
		       &limbDark, obsmagInfoArr, numObsmag, colorInfoArr, numColors,  starGroup, numGroups,
		       argv[5], deb_filenamePeriod, foutResults) ;
	}
      else
	{
#ifdef SET_ZERO_ECCENTRICITY
	  ...
	  if (!foutFit || !foutDat)
	    {
	      ...
	      if (foutFit) fclose (foutFit) ;
	      if (foutDat) fclose (foutDat) ;
	    }
	}
      else // LCweighting == 0.0
	printError (argv[5], 0, deb_filenamePeriod, "main()", "Since the light curve was ignored, no fit file was written.") ;
      ...
      getScore (best_g, best_i, best_j, indexLCfilter, period, splineScore, time, amp, err, size,
		combObsMag, ecc, t0, omega, LCweighting, midTime, &limbDark, colorInfoArr, numColors, starGroup, 
  ...
  printf ("\nCreated files:\n") ;
  printf ("  %s - output result file\n", argv[4]) ;
  printf ("  %s - output error file\n", argv[5]) ;
#ifdef WRITE_MODEL_FIT_LC
  printf ("  *%s - fitted light curve models for the best mass-pairing of each LC\n", WRITE_MODEL_FIT_LC) ;
  printf ("  *%s - light curve data and model residuals for the best mass-pairing of each LC\n", WRITE_MODEL_DAT_LC) ;
#endif
#ifdef CONTOUR_OUTPUT_SUFFIX
  printf ("  *%s - contour data files\n", CONTOUR_OUTPUT_SUFFIX) ;
  printf ("  %s - IDL script for plotting the contour data\n", filenameIDL) ;
  printf ("\nTo run the IDL script, enter: 'idl', then '.run %s', then 'exit'.\n", filenameIDL) ;
#endif
